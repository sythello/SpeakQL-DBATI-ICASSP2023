...
from allennlp.data.fields import TextField, LabelField, SequenceLabelField, ArrayField, MetadataField, ListField, NamespaceSwappingField
...
@DatasetReader.register('spider_ASR_reranker_reader_v2_comb_new')
class SpiderASRRerankerReaderV2_Combined_new(SpiderASRFixerReader_Base):
    '''
    V2: Audio features are passed as sequences, not avg/max pooled vectors.
    This is not Siamese (implementation of Siamese reranker is non-trivial in the current framework... need to refactor base class)
    '''
    def __init__(self,
                 ...
                 cands_limit: int = None,                   # Only using this number of cands per sample
                 for_training: bool = True,             # True for training (skip samples with all cands equally good); False for inference (don't skip)
                 debug: bool = False,
        ...
        if cands_limit is not None:
            assert cands_limit > 1, "Reranker can't have cands_limit = 1"
    ...
    def text_to_instance(self,
        ...
        if oracle_label is not None:
            fields["oracle_label"] = LabelField(oracle_label, skip_indexing=True)
    ...
    def _text_to_instance_specific_proc(self, cand, text_to_instance_kwargs, cand_list=None) -> bool:
        cand_sort_index = lambda _c: (int(_c['ratsql_pred_score'] * 1e8), _c['question_toks_edit_distance'])

        # score_list = [int(_c['ratsql_pred_score'] * 1e8) for _c in cand_list]   # *1e8 to avoid float precision diff
        # edist_list = [_c['question_toks_edit_distance'] for _c in cand_list]

        ## (score: high -> low; edist: low -> high) 
        _l = sorted([cand_sort_index(_c) for _c in cand_list], key=lambda p: (-p[0], p[1]))
        if _l[0] == _l[-1] and self.for_training:
            # do not use for training; keep it for prediction 
            return False

        if cand_sort_index(cand) == _l[0]:
            # is best
            oracle_label = 1
        else:
            oracle_label = 0

        # print(cand['question'], cand_sort_index(cand))
        # print('Full cand_list:', _l)
        # print('Oracle label:', oracle_label)
        
        text_to_instance_kwargs['oracle_label'] = oracle_label
        return True
