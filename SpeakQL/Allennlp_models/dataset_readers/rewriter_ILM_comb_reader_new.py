...
from allennlp.data.fields import TextField, LabelField, MultiLabelField, SequenceLabelField, ArrayField, MetadataField, ListField, NamespaceSwappingField
...
@DatasetReader.register('spider_ASR_rewriter_reader_ILM_comb_new')
class SpiderASRRewriterReader_ILM_Combined_new(SpiderASRFixerReader_Base):
    '''
    Sequence tagging + ILM rewrite generation, ILM part
    '''
    def __init__(self,
                 ...
                 use_full_sent_as_rewrite_seq: bool = False, # Predict the full target sentence (like s2s), instead of only rewrite spans
        ...
        self.use_full_sent_as_rewrite_seq = use_full_sent_as_rewrite_seq
    ...
    def text_to_instance(self,
        ...
        ## Casing check: making sure our assumptions about special tokens correctly
        # for t in metadata["concat_tokens"]:
        #     if t[0] in '[@':
        #         ## single punct or special token
        #         assert t in {'[ANS]', '[CLS]', '[SEP]', '@@PADDING@@', '@@UNKNOWN@@', START_SYMBOL, END_SYMBOL} or t.lower() == t, t
        ## No: Databases have weird cells, like "[Disc", "[Just", "[Acoustic"

        text_len = metadata["text_len"]
        schema_len = metadata["schema_len"]
        concat_len = metadata["concat_len"]
        concat_tokens = [Token(t) for t in metadata["concat_tokens"]]   ## List[Token]
        ...
        concat_src_to_tgt_field = NamespaceSwappingField(concat_tokens_lower, "tgt_tokens")   # Hardcoded - tgt_token_indexer must use namespace "tgt_tokens"!
        fields["source_to_target"] = concat_src_to_tgt_field
        ...
        if rewrite_tokens is not None:
            ...
            metadata["target_tokens"] = rewrite_tokens[1:-1]
            metadata["rewrite_seq_len"] = len(rewrite_tokens[1:-1])

            ## copied from s2s; add lower
            token_ids = self._token_to_ids(concat_tokens_lower + rewrite_tokens)
            source_token_ids = token_ids[:len(concat_tokens_lower)]
            target_token_ids = token_ids[len(concat_tokens_lower):]
            fields["source_token_ids"] = ArrayField(np.array(source_token_ids))
            fields["target_token_ids"] = ArrayField(np.array(target_token_ids))
        else:
            source_token_ids = self._token_to_ids(concat_tokens_lower)
            fields["source_token_ids"] = ArrayField(np.array(source_token_ids))
    ...
    def _text_to_instance_specific_proc(self, cand, text_to_instance_kwargs, cand_list=None) -> bool:
        ...
        if not self.include_gold_rewrite_seq:
            rewrite_tokens = None
        elif not self.use_full_sent_as_rewrite_seq:
            ## Regular, only the rewrite spans 
            # rewrite_list = ['[START_RWT]']
            rewrite_list = []
            for _edit in cand['rewriter_edits']:
                rewrite_list.extend(_edit['tgt_text'].lower().split(' '))
                rewrite_list.append('[ANS]')
            # rewrite_list.append('[END_RWT]')
            rewrite_tokens = [Token(w) for w in rewrite_list]
        else:
            ## Compare, use the full target sentence 
            rewrite_tokens = [Token(word.lower()) for word in cand['gold_question_toks']]
        
        text_to_instance_kwargs['rewriter_tags'] = rewriter_tags
        text_to_instance_kwargs['rewrite_tokens'] = rewrite_tokens
