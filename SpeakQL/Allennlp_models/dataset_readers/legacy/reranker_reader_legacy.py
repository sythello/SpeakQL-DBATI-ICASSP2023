...
@DatasetReader.register('spider_ASR_reranker_reader_v1')
class SpiderASRRerankerReaderV1(DatasetReader):
    '''
    V1: Audio features are passed as vectors, from avg/max pooling.
    '''
    def __init__(self,
    ...
    def text_to_instance(self,
                         ...
                         text_audio_feats: np.ndarray,
                         schema_audio_feats: np.ndarray,
        ...
        concat_audio_feats = np.concatenate((text_audio_feats, np.zeros((1, AUDIO_DIM_POOLING)), schema_audio_feats), axis=0)
        ...
        concat_audio_field = ArrayField(concat_audio_feats)
        ...
        if score is not None:
            score_field = ArrayField(array=np.array([score]))
    ...
    def _read(self, file_path: str) -> Iterator[Instance]:
        ...
        # dataset_json_path = os.path.join(self.dataset_dir, sub_dir, '{}_reranker.json'.format(ds))
        # audio_fname_tmpl = os.path.join(self.dataset_dir, sub_dir, 'speech_wav', '{}.wav')
        ...
        for i, cand_list in enumerate(dataset_json):
            ...
            if len(cand_list) <= 1:
                # 0 or 1 cands, no ranking applicable 
                continue
            ...
            db_id = cand_list[0]['db_id']
            db_schema = databases[db_id]
            # db_fname = db_fname_tmpl.format(cand_list[0]['db_id'])  # Maybe should combine the common fields for all candidates, like original_id, db_id, etc. 
            # print(db_fname)
            # db_schema = read_dataset_schema(db_fname)
            ...
            for token in schema_sentence:
                # print('<{}>'.format(token))
                if token in ',.:':
                    schema_audio_feats.append(np.zeros(AUDIO_DIM_POOLING))
                else:
                    feats_fname = os.path.join(self.dataset_dir, 'db', 'speech_feats', '{}.pkl'.format(token))
                    with open(feats_fname, 'rb') as f:
                        feats_vec = pickle.load(f)
                    schema_audio_feats.append(feats_vec)
            schema_audio_feats_array = np.vstack(schema_audio_feats)
            assert len(schema_audio_feats_array) == len(schema_sentence)
            ...
            for cand in cand_list:
                ...
                text_audio_feats_array = extractAudioFeatures(audio_fname, cand['span_ranges'])
                score = (cand['ratsql_pred_score'] - min_score) / (max_score - min_score)
                yield self.text_to_instance(o_id,
                                            text_tokens,
                                            schema_tokens,
                                            text_audio_feats_array,
                                            schema_audio_feats_array,
                                            score)


@DatasetReader.register('spider_ASR_reranker_reader_v2')
class SpiderASRRerankerReaderV2(DatasetReader):
    '''
    ...
    def __init__(self,
                 ...
                 token_indexers: Dict[str, TokenIndexer] = None,
                 max_sequence_len: int = 300,
                 skip_unrankable_samples: bool = True,  # Should be true for training, false for inference
                 debug: bool = False) -> None:
        super().__init__(lazy=False)
        self.tables_json_fname = tables_json_fname
        self.dataset_dir = dataset_dir
        ...
        self.token_indexers = token_indexers
        self.max_sequence_len = max_sequence_len
        self.skip_unrankable_samples = skip_unrankable_samples
        self.debug = debug
    ...
    def _read(self, file_path: str) -> Iterator[Instance]:
        ...
        for i, cand_list in enumerate(dataset_json):
            ...
            if len(cand_list) == 1 and self.skip_unrankable_samples:
            ...
            max_score = max(score_list)
            min_score = min(score_list)
            if np.isclose(max_score, min_score) and self.skip_unrankable_samples:
            ...
            for cand in cand_list:
                ...
                # score = (cand['ratsql_pred_score'] - min_score) / (max_score - min_score)     # rescaling
                score = cand['ratsql_pred_score']
                yield self.text_to_instance(o_id,
                                            text_tokens,
                                            schema_tokens,
                                            text_audio_feats_list,
                                            schema_audio_feats_list,
                                            score)
