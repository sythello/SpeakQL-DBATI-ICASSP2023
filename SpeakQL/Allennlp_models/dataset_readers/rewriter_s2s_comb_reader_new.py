...
@DatasetReader.register('spider_ASR_rewriter_reader_seq2seq_comb_new')
class SpiderASRRewriterReader_Seq2seq_Combined_new(SpiderASRFixerReader_Base):
    '''
    Seq2seq baseline, with copy mechanism; can choose if using BERT / TaBERT embedder, align tags, etc.
    '''
    def __init__(self,
                 ...
                 target_lowercasing: bool = False,      # If true, lowercase target and copy-related fields; if false, don't lowercase anything
                 ## probes
        ...
        self.target_lowercasing = target_lowercasing
    ...
    def text_to_instance(self,
        ...
        # concat_tokens_for_copy: follow target casing 
        if self.target_lowercasing:
            concat_tokens_for_copy = [Token(self._maybe_lower(t)) for t in metadata["concat_tokens"]]
        else:
            concat_tokens_for_copy = concat_tokens

        metadata["source_tokens"] = concat_tokens_for_copy

        concat_src_to_tgt_field = NamespaceSwappingField(concat_tokens_for_copy, "tgt_tokens")   # Hardcoded - tgt_token_indexer must use namespace "tgt_tokens"!
        ...
        if rewrite_tokens is not None:
            ...
            metadata["target_tokens"] = [t.text for t in rewrite_tokens[1:-1]]
            
            token_ids = self._token_to_ids(concat_tokens_for_copy + rewrite_tokens)
            source_token_ids = token_ids[:len(concat_tokens_for_copy)]
            target_token_ids = token_ids[len(concat_tokens_for_copy):]
            fields["source_token_ids"] = ArrayField(np.array(source_token_ids))
            fields["target_token_ids"] = ArrayField(np.array(target_token_ids))
        else:
            source_token_ids = self._token_to_ids(concat_tokens_for_copy)
            fields["source_token_ids"] = ArrayField(np.array(source_token_ids))
