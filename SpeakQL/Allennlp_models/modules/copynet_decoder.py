...
@SeqDecoder.register("speakql_copynet_seq_decoder")
class CopyNetSeqDecoder(SeqDecoder):
    """
    ...
    def __init__(
        ...
        encoder_output_dim: int,        # @YS
        is_bidirectional_input: bool,   # @YS
        attention: Attention,
        beam_size: int,
        max_decoding_steps: int,
        # target_embedding_dim: int = 30,
        target_embedder: Embedding,     # @YS
        copy_token: str = "@COPY@",
        target_namespace: str = "tgt_tokens",
        tensor_based_metric: Metric = None,
        token_based_metric: Metric = None,
        initializer: InitializerApplicator = InitializerApplicator(),
    ) -> None:
        super().__init__(target_embedder)               # @YS

        self._vocab = vocab                             # @YS
        ...
        self._pad_index = self._vocab.get_token_index(
        ...
        self.target_embedder.extend_vocab(self._vocab)  # @YS
        ...
        # print(self.target_embedder.num_embeddings)
        # print(self._vocab.get_index_to_token_vocabulary("tgt_tokens"))
        # print('='*30)
        ...
        self.target_embedding_dim = self.target_embedder.get_output_dim()   # @YS
        # self._target_embedder = Embedding(
        #     num_embeddings=self._target_vocab_size, embedding_dim=target_embedding_dim
        # )
        self._attention = attention
        self._input_projection_layer = Linear(
            self.target_embedding_dim + self.encoder_output_dim * 2, self.decoder_input_dim
        )
        ...
        # @YS: Save intermediates for interpretation
    ...
    def _maybe_save(self, val, name):
        if self.save_intermediate:
            if torch.is_tensor(val):
                ...
                print(f"Warning: saved '{name}' is not a tensor")
    ...
    def forward(
        ...
        source_mask: torch.Tensor,      # @YS
        encoder_outputs: torch.Tensor,  # @YS
        ...
        # @YS
        state = {"source_mask": source_mask, "encoder_outputs": encoder_outputs}
            ...
            if target_tokens:
                if self._tensor_based_metric is not None:
                    ...
                    gold_tokens = self._gather_extended_gold_tokens(
                        target_tokens["tgt_tokens"]["tokens"], source_token_ids, target_token_ids
                    )
    ...
    def _init_decoder_state(self, state: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        final_encoder_output = util.get_final_encoder_states(
            ...
            self.is_bidirectional_input # @YS
        )
    ...
    def _decoder_step(
        ...
        # print([self._vocab.get_token_from_index(idx, namespace="tgt_tokens") for idx in last_predictions.numpy()])
        # print('-'*20)
        # print(self.target_embedder.num_embeddings)
        # print('='*30)
    ...
    def _forward_loss(
        ...
        """
        batch_size, target_sequence_length = target_tokens["tgt_tokens"]["tokens"].size()
        ...
        for timestep in range(num_decoding_steps):
            # shape: (batch_size,)
            input_choices = target_tokens["tgt_tokens"]["tokens"][:, timestep]
            ...
            # shape: (batch_size,)
            step_target_tokens = target_tokens["tgt_tokens"]["tokens"][:, timestep + 1]
            ...
            ## @YS: save intermediate
            self._maybe_save(input_choices, f'input_choices:step_{timestep}')
            self._maybe_save(generation_scores, f'generation_scores:step_{timestep}')
            self._maybe_save(copy_scores, f'copy_scores:step_{timestep}')
            self._maybe_save(step_target_tokens, f'step_target_tokens:step_{timestep}')
            self._maybe_save(state['decoder_hidden'], f'state_decoder_hidden:step_{timestep}')
            self._maybe_save(state['decoder_context'], f'state_decoder_context:step_{timestep}')
    ...
    def _forward_beam_search(self, state: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        # @YS TODO: use modified BeamSearch to save intermediates, i.e. attention and selection weight vectors per step
