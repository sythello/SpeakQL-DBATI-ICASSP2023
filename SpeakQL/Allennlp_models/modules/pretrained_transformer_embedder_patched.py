...
@TokenEmbedder.register("pretrained_transformer_patched")
class PretrainedTransformerEmbedder_Patched(TokenEmbedder):
    """
    ...
    @YS changes:
    1. update config -> sub_module config (when sub_module is used)
    2. "scalar_mix_size": allow passing num of hidden layers (for scalar mix)
    3. "set_output_hidden_states": allow setting "output_hidden_states" when calling transformer model, when it's specified
    4. "hidden_states_return_idx": pointer to the hidden states in the return of transformer_model()
    ...
    def __init__(
        ...
        *,
        ...
        # @YS add
        scalar_mix_size: int = None,
        set_output_hidden_states: bool = None,
        hidden_states_return_idx: int = None,
    ) -> None:
        super().__init__()
        ...
        # self.config = self.transformer_model.config  # This doesn't work
        ...
        if not last_layer_only:
            # @YS
            _scalar_mix_size = scalar_mix_size or self.config.num_hidden_layers
            # self._scalar_mix = ScalarMix(self.config.num_hidden_layers)
            self._scalar_mix = ScalarMix(_scalar_mix_size)
        ...
        # @YS add
        self.scalar_mix_size = scalar_mix_size
        self.set_output_hidden_states = set_output_hidden_states
        self.hidden_states_return_idx = hidden_states_return_idx if hidden_states_return_idx is not None else -1
    ...
    def forward(
        ...
        # @YS
        if self.set_output_hidden_states is not None:
            parameters["output_hidden_states"] = self.set_output_hidden_states
        ...
        if self._scalar_mix is not None:
            ...
            hidden_states = transformer_output[self.hidden_states_return_idx][1:]  # @YS
